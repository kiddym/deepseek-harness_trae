...
- 当前状态：交付收口返工已完成：CI 解析与失败产物上传、服务重启持久化、设计实现一致性、前端请求状态、浏览器路径、停止判据和取消删除均已处理。
- 范围状态：第一期范围已冻结；登录、多用户、权限、文件上传、外部 API、SSR、微服务、桌面安装包、收费与部署均明确排除。
- 风险状态：本地 13 个 Playwright 场景全部通过，三态退出码（0/1/3）各构造反例通过，服务重启持久化以及通过态与失败态 artifact 留存验证通过；工作流 `.github/workflows/ci.yml` 已在 `ubuntu-latest` 上多次成功运行，含 35206609960（13 passed，`playwright-test-artifacts` 非空上传，184155 字节）。每次推送都会触发 CI，最新结果以仓库 Actions 记录为准，本计划不再逐次枚举。早期 0 秒失败源于工作流文件本身（job 级 `env` 使用了 `runner` 上下文，GitHub 拒绝解析），已修复；35204856053 的上传步骤曾因脚本收尾删除产物而为空转，已修复并在后续运行中验证非空上传。

## 4. 本工作包产物与验证

- Playwright 与 `playwright` 均锁定为 1.49.1。
- `playwright.config.ts` 使用 `channel: "chromium"`，浏览器路径由 `PLAYWRIGHT_BROWSERS_PATH` 提供。
- `tests/tasks.spec.ts` 包含 13 个独立中文场景，覆盖核心流程、异常路径、接口边界和默认数据目录隔离。
- `.github/workflows/ci.yml` 在 Ubuntu、Node 22 上执行依赖安装、Chromium 安装、构建和统一测试脚本。
- 本地验证结果：13 个 Playwright 场景全部通过；三态退出码（0/1/3）各构造反例通过；服务重启持久化通过；通过态 artifact 保留 `results.json`、HTML 报告和 `.last-run.json`，失败态另保留失败用例的 `trace.zip`。
...
本地 13 个 Playwright 场景全部通过，三态退出码（0/1/3）各构造反例通过，服务重启持久化和通过态与失败态 artifact 留存验证均已完成；CI 工作流已在 `ubuntu-latest` 上多次成功运行，含 35206609960（13 passed，`playwright-test-artifacts` 非空上传 184155 字节）。每次推送都会触发，最新结果以仓库 Actions 记录为准，本计划不再逐次枚举。
...
- 当前验证：`npm run build` 通过，`npm test` 自动先构建并通过 13/13 用例。

### 下一步

本工作包交付收口完成，待 Owner/planner 决定下一步。
...
本工作包交付收口完成，待 Owner/planner 决定下一步；每次推送产生的新 CI 结果以仓库 Actions 记录为准。
...
已构造并运行通过态（退出 0，12 passed）、断言失败态（退出 1）、未设置浏览器路径态（退出 3）和服务未就绪态（退出 3）；README 已记录构造方式和预期输出。CI 工作流已在 `ubuntu-latest` 上多次成功运行，最新结果以仓库 Actions 记录为准。

## 9. 交付收口返工工作包

### 状态

已完成。本轮增加 CI 失败时的 artifact 上传、记录 CI 多次成功运行和此前 0 秒 workflow 解析失败，并新增取消删除的独立真实界面场景。

### 验证安排

本地 13 个 Playwright 场景、三态退出码反例、通过/失败两种 artifact 保留验证和服务重启验证均已完成；CI 工作流已在 `ubuntu-latest` 上多次成功运行，含 35206609960（13 passed，`playwright-test-artifacts` 非空上传 184155 字节）。每次推送都会触发，最新结果以仓库 Actions 记录为准，本计划不再逐次枚举；本工作包交付收口完成，待 Owner/planner 决定下一步。
