...
try { const ready = spawn(process.execPath, ['scripts/ready.mjs'], { env: { ...process.env, PORT: String(port), READY_TIMEOUT_MS: '30000' }, stdio: 'inherit' }); if ((await new Promise((resolve) => ready.on('exit', resolve))) !== 0) throw new Error('测试服务未就绪'); const test = spawn('npx', ['playwright', 'test'], { env: { ...process.env, PORT: String(port), TEST_ARTIFACT_DIR: artifactDir, PLAYWRIGHT_BROWSERS_PATH: process.env.PLAYWRIGHT_BROWSERS_PATH || path.join(process.env.HOME || process.env.USERPROFILE || tmpdir(), 'Documents/Projects/.wp05-pw-browsers') }, stdio: 'inherit' }); code = await new Promise((resolve) => test.on('exit', resolve)); } finally { child.kill('SIGTERM'); await new Promise((resolve) => child.on('exit', resolve)); try { rmSync(testRoot, { recursive: true, force: true }); } catch {} }
process.exit(code);
